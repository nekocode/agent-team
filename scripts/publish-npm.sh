...
sync_versions() {
    ...
    for pkg_dir in "$NPM_DIR"/agent-team*/; do
    ...
    local main_pkg="$NPM_DIR/agent-team/package.json"
    for platform in darwin-arm64 darwin-x64 linux-x64 win32-x64; do
        if [[ "$(uname)" == "Darwin" ]]; then
            sed -i '' "s/\"@nekocode\/agent-team-$platform\": \".*\"/\"@nekocode\/agent-team-$platform\": \"$version\"/" "$main_pkg"
        else
            sed -i "s/\"@nekocode\/agent-team-$platform\": \".*\"/\"@nekocode\/agent-team-$platform\": \"$version\"/" "$main_pkg"
        fi
    done
...
for platform in darwin-arm64 darwin-x64 linux-x64 win32-x64; do
    pkg_dir="$NPM_DIR/agent-team-$platform"
    pkg_name="@nekocode/agent-team-$platform"
    if [[ "$platform" == "win32-x64" ]]; then
        binary="$pkg_dir/bin/agent-team.exe"
    else
        binary="$pkg_dir/bin/agent-team"
    fi
...
publish_package "$NPM_DIR/agent-team" "agent-team" "$VERSION" "$DRY_RUN"
